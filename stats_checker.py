    ...
    while current_block <= current_network_block:
        ...
        if len(txs_in_block) > 0:
            for tx in txs_in_block:
                addresses = save_transfer(api, tx, network)
                if network == 'DUCX':
                    addresses_in_txes.append(addresses.get('address_from'))
                    addresses_in_txes.append(addresses.get('address_to'))

        print(f'Block: {current_block}, tx count: {len(txs_in_block)}')
        current_block += 1
