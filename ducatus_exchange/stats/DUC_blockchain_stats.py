...
from ducatus_exchange.stats.LastBlockPersister import get_last_DUC_block
...
def get_last_DUC_blockchain_block():
    req_string = 'https://ducapi.rocknblock.io/api/DUC/mainnet/block/'
...
def get_block_transactions(block_number):
    req_string = 'https://ducapi.rocknblock.io/api/DUC/mainnet/tx?blockHeight=' + str(block_number)
...
def get_transactions_from_blocks(now_block, scan_until_block):
    txs = []
    current_block = now_block

    while current_block <= scan_until_block:
        txs_in_block = get_block_transactions(current_block)
...
if __name__ == '__main__':
    ...
    STOP_BLOCK = get_last_blockchain_block()
    api = DucatusAPI()
    txs = get_transactions_from_blocks(CURRENT_BLOCK, STOP_BLOCK)
    total_txs = len(txs)
    ...
    for t in txs:
        ...
        total_value += int(get_tx_value(api, t))
        t_num += 1
